  ...
  function TableauEquipement({ refresh, search }) {
    ...
    const fetchEquipements = async (page) => {
      ...
      try {
        const response = await fetch(
          `http://localhost:8000/api/equipements?page=${page}`,
          {
            headers: {
    ...
    const filtered = equipements.filter((item) =>
      item.nom_equipement.toLowerCase().includes((search ?? "").toLowerCase()),
    );
            ...
            <tbody>
              ...
              {filtered.map((equipement, index) => (
                ...
                >
                  ...
                  <td className="px-4 lg:px-6 py-4">{equipement.nom_equipement}</td>
                  <td className="px-4 lg:px-6 py-4">{equipement.nombre_equipement}</td>
